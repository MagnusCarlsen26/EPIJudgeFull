...
def parse_problem_mapping(mapping_path: Path) -> dict:
    text = read_text_or_head(mapping_path)
    text = text.strip()
...
def read_text_or_head(path: Path) -> str:
    if path.exists():
        return path.read_text(encoding="utf-8")
    repo_root = path.parent
    for revision in ("HEAD", "6cc4002^"):
    ...
    raise FileNotFoundError(f"Could not read {path.name} from disk or git history")
